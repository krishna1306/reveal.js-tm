...
function start_eval(green_time, yellow_time, red_time, grace_time) {
  ...
  bell_wrapper = setTimeout(function() {
    bell.play();
    document.getElementById("elapsed_eval").innerHTML = "Time exceeded 🛎";
    bell_sound = setInterval(function() {
      bell.play();
    }, 10000);
  }, grace_time);

  document.getElementById("elapsed_eval").innerHTML = "Timer is running 🏃‍♀️";
}
...
function stop_eval() {
  ...
  if (t4 > 0){
    ...
    var seconds = (t5 - t4) / 1000;
    var mins = seconds / 60;
    seconds = seconds % 60;

    document.getElementById("elapsed_eval").innerHTML = Math.floor(mins) + " mins " + Math.round(seconds) + " seconds.";
    ...
    document.getElementById("elapsed_eval").innerHTML = "Timer not started yet 🤷‍♂️";
    clearInterval(bell_sound);
    clearTimeout(bell_wrapper);
  }
}
...
function reset_eval() {
  ...
  document.getElementById("elapsed_eval").innerHTML = "";
